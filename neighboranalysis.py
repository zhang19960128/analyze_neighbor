...
print np.divide(sum,len(balist))
sum=[0,0,0];
for i in sin:
	sum=np.add(sum,data[i]);
print np.divide(sum,len(sin));
sum=[0,0,0];
count=0;
for i in dou:
	if i[2]==0:
		count=count+1;
		sum=np.add(sum,data[i[1]]);
		sum=np.add(sum,data[i[2]]);
print np.divide(sum,count*2);
print count
sum=[0,0,0];
count=0;
for i in dou:
